    ...
    handlers: {
        ...
        dice: function(user, params, { rank }) {
            this.checkCooldown({
                ...
                params = params.replace(/-team[a-zA-Z0-9]*-/g, '').trim();
                // Check for 'comments'
                if(params.match(/#|\/\//)){
                    switch(true){
                ...
                if (roll.rolls && roll.rolls.length > 0) {
                    const allRolls = roll.rolls.flat();
                    if (allRolls.length > 1) {
                        rollString = ` (${allRolls.join(', ')})`;
                    }
                }
                this.sendMessage(`[Dice] ${user}: ${roll.total}${rollString}`);
            },(message)=>{
        ...
        slap: function(user, params, { rank }) {
            if (!params) { return }

            this.checkCooldown({
                type: 'rng', user, modBypass: rank > 2
            }).then(()=>{
                this.sendMessage(`${user} slaps ${params} around with a large trout!`);
            },(message)=>{
                this.sendPrivate(message, user);
            });
        },
