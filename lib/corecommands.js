...
module.exports = {
    handlers: {
        ...
        betreset: function(user, params, meta) {
            this.checkPermission({
                user, rank: 3, hybrid: 'betadmin'
            }).then(()=>{
                this.betting.ledger = {};
                this.betting.saveLedger();
                this.sendMessage('Betting ledger has been reset.');
            },(message)=>{
                this.sendPrivate(message, user);
            });
        },

        topbets: function(user, params, meta) {
            const top = Object.entries(this.betting.getLedger())
                .sort(([,a],[,b]) => b-a)
                .slice(0, 5)
        ...
        forceclose: function(user, params, meta) {
            this.checkPermission({
                user, rank: 2, hybrid: 'betadmin'
            }).then(()=>{
        ...
        forcewinner: function(user, params, meta) {
            this.checkPermission({
                user, rank: 2, hybrid: 'betadmin'
            }).then(()=>{
