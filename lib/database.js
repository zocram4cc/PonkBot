...
class PonkDatabase extends EventEmitter {
    ...
    updateTables(){
        ...
        this.getKeyValue('dbversion').then((version) => {
            ...
            state++; // 3
            if (this.version === state) {
                this.logger.log('Creating tables for GT4CC race results.')
                return this.knex.schema.createTableIfNotExists('gt4cc_races', (table) => {
                    table.increments('id').primary();
                    table.integer('timestamp').unsigned();
                    table.string('name');
                })
                .createTableIfNotExists('gt4cc_results', (table) => {
                    table.integer('race_id').unsigned().references('id').inTable('gt4cc_races').onDelete('CASCADE');
                    table.integer('position').unsigned();
                    table.string('driver');
                    table.integer('points');
                    table.primary(['race_id', 'position']);
                })
                .then(()=>{
                    this.setKeyValue('dbversion', ++state).then(()=>{
                        this.updateTables();
...
Object.assign(PonkDatabase.prototype, {
    ...
    messageInsert: function({ timestamp, user, message, channel }){
        this.knex('chat').insert({ timestamp, user, message, channel })
            .catch(error => {
                this.logger.error('Unexpected error', '\n', error);
            });
    },
...
*/
Object.assign(PonkDatabase.prototype, {
    gt4ccSaveRace: function(timestamp, name, results) {
        return this.knex.transaction(async (trx) => {
            const ids = await trx('gt4cc_races').insert({ timestamp, name });
            const race_id = ids[0];
            // results should be an array of { position, driver, points }
            const entries = results.map(r => ({ ...r, race_id }));
            await trx('gt4cc_results').insert(entries);
            return race_id;
        });
    },

    gt4ccDeleteRace: function(id) {
        return this.knex.transaction(async (trx) => {
            await trx('gt4cc_results').where({ race_id: id }).del();
            await trx('gt4cc_races').where({ id }).del();
        });
    },

    gt4ccGetRaces: function() {
        return this.knex('gt4cc_races').orderBy('timestamp', 'desc');
    },

    gt4ccGetRaceResults: function(race_id) {
        return this.knex('gt4cc_results')
            .where({ race_id })
            .orderBy('position', 'asc');
    },

    gt4ccGetChampionship: function() {
        return this.knex('gt4cc_results')
            .select('driver')
            .sum('points as total_points')
            .count('race_id as races_participated')
            .groupBy('driver')
            .orderBy('total_points', 'desc');
    },

    gt4ccGetConstructors: function() {
        const DRIVER_TO_MAKE = {
            'Hitler': 'Alfa Romeo',
            '/I/Rene': 'Alfa Romeo',
            'Giantess': 'Alfa Romeo',
            'Kaworu Nagisa': 'Alfa Romeo',
            'Charles II': 'BMW',
            'Real Human Being': 'BMW',
            'Gimi': 'BMW',
            'Sex Tourist': 'BMW',
            'G-TEK': 'Ford',
            'KWAB': 'Ford',
            'Shinichi Mochizuki': 'Ford',
            'Take It To The Vet': 'Ford',
            'F99 Record Breaker': 'Honda',
            'TEH REI': 'Honda',
            'Get Out Of /Jp/': 'Honda',
            'Doomguy': 'Honda',
            'Shygal': 'Renault',
        ...
        return this.knex('gt4cc_results')
            ...
            .then(results => {
                const constructors = {};
                results.forEach(row => {
                    const make = DRIVER_TO_MAKE[row.driver];
                    if (make) {
                        if (!constructors[make]) {
                            constructors[make] = { make, total_points: 0, drivers: new Set() };
                        }
                        constructors[make].total_points += row.points;
                        constructors[make].drivers.add(row.driver);
                    }
                });
                ...
                return Object.values(constructors)
                    .map(c => ({
                        ...
                        driver_count: c.drivers.size
                    }))
                    .sort((a, b) => b.total_points - a.total_points);
            });
    },

    gt4ccUpdateRaceTitle: function(id, name) {
        return this.knex('gt4cc_races')
            .where({ id })
            .update({ name });
    },

    gt4ccExchangeResults: function(race_id, pos1, pos2) {
        return this.knex.transaction(async (trx) => {
            const results = await trx('gt4cc_results')
                .where({ race_id })
                .whereIn('position', [pos1, pos2]);

            if (results.length !== 2) {
                throw new Error('Could not find both positions for the given race');
            }

            const res1 = results.find(r => r.position === pos1);
            const res2 = results.find(r => r.position === pos2);

            await trx('gt4cc_results')
                .where({ race_id, position: pos1 })
                .update({ driver: res2.driver });

            await trx('gt4cc_results')
                .where({ race_id, position: pos2 })
                .update({ driver: res1.driver });
        });
    },
});
...
//                 .then((result)=>{
//                     if(result.length){
//                         return resolve((result.pop())['odds']);
//                     }
//                     return resolve(undefined);
//                 },(error)=>{
//                     this.logger.error('Unexpected error', '\n', error);
...
//     insertTeam: function(name, odds = 1.5){
//         return new Promise((resolve, reject)=>{
//             this.knex('teams')
//                 .insert({ name, odds })
//                 .then((result)=>{
...
//     updateTeamOdds: function(name, odds){
//         return new Promise((resolve, reject)=>{
//             this.knex('teams')
//                 .where({ name })
//                 .update({ odds })
