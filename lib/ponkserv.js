...
class PonkServer extends EventEmitter {
	...
	this.host = this.setupExpress(express(), {
	    ...
	    session:    session,
	    flash:      require("connect-flash"),
	    store:      ConnectSessionKnexStore,   // ✅ pass the class directly
	},{
   	    secret: this.secret
	});
        ...
        // Betting API
        this.setupBettingApi(this.host);

        // GT4CC API
        this.setupGt4ccApi(this.host);

        // Scheduler/Planner
        if(this.bot.scheduler.enabled){
            this.setupPlanner(this.host);
        }
    ...
    setupRoutes(app){
        ...
        // TODO: show system, movie system, emotes, and custom pages (stream help)
    ...
    setupBettingApi(app) {
        this.logger.log('Registering betting API routes.');
        const betapi = require('../routes/betapi.js')(this.bot);
        app.use('/api', betapi);
    }

    setupGt4ccApi(app) {
        this.logger.log('Registering GT4CC API routes.');
        const gt4ccapi = require('../routes/gt4cc.js')(this.bot);
        app.use('/api/gt4cc', gt4ccapi);
    }
    ...
    setupPlanner(app){
        ...
        app.get('/scheduler', (req, res)=>{
            this.logger.debug(`Sending page: scheduler.`)
            this.sendPug(res, 'scheduler', {
                user: req.user,
                serverIO: this.socketlink
            });
        });
...
class PonkSocket {
    ...
    setupSocket(io, port){
        ...
        this.setupListeners(io_server).use((socket, next)=>{
    ...
    setupListeners(io){
        ...
        io.sockets.on('connection', (socket)=>{
            ...
            socket.on('getSchedule',   ()=>{ this.getSchedule(socket) });
            ...
            socket.on('removeJob', (query)=>{ this.removeJob(socket, io, query) });
    ...
    getSchedule(socket){
        this.bot.scheduleGet().then(jobs => { socket.emit('schedule', jobs) })
            .catch(problem => {
                switch(problem.message){
                    default:
                        socket.emit('problem', 'There was a problem retrieving the schedule.');
                }
            });
    }

    removeJob(socket, io, query){
        this.bot.scheduleRemove(query).then((result) => {
            io.sockets.emit('removal', result);
            socket.emit('removalSuccess', true);
        },(error) => {
            socket.emit('removalFailure', error.message);
        });
    }
}
