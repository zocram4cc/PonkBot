...
function updateTournamentInfo() {
  fetch('/api/tournament')
    ...
    .then(data => {
      ...
      if (tournament.choices.length === 0) {
        infoDiv.innerHTML = '<p>No tournament active.</p>';
        totalsDiv.style.display = 'none';
      ...
      if (!tournament.open) {
        ...
        infoDiv.innerHTML = `<p>Tournament betting is open! Choices: ${tournament.choices.join(', ')}</p>`;
      }
